...
class L_Reconstruction(nn.Module):
    def __init__(self, losstype='l2', eps=1e-6):
        super(L_Reconstruction, self).__init__()
...
@LOSS_REGISTRY.register()
class L_MarginRanking(nn.Module):
    def __init__(self, margin=0.1, target=1, loss_weight=1, reduction='mean'):
        super(L_MarginRanking, self).__init__()
        self.margin = margin
        self.reduction = reduction
        self.weight = loss_weight
        self.target = target

    def forward(self, recon_H, gt, recon_L, lq):
        b, _, _, _ = recon_H.shape
        recon_H = recon_H.view(b, -1)
        gt = gt.view(b, -1).detach()
        recon_L = recon_L.view(b, -1)
        lq = lq.view(b, -1)

        person_H = torch.cosine_similarity(recon_H, gt, dim=0)
        person_L = torch.cosine_similarity(recon_L, lq, dim=0)
        self.target = torch.ones_like(person_L) * self.target

        # l = max(0, −target ∗ (x1 − x2) + margin)  while target = 1, x1 > x2
        return self.weight * F.margin_ranking_loss(person_H, person_L, self.target, margin=self.margin, reduction=self.reduction)


@LOSS_REGISTRY.register()
class L_CosSimiliarity(nn.Module):
    def __init__(self, loss_weight=1):
        super(L_CosSimiliarity, self).__init__()
        self.weight = loss_weight
    ...
    def forward(self, recon_H, gt, recon_L, lq):
        ...
        ones = torch.ones_like(person_L)
        # l = max(0, −target ∗ (x1 − x2) + margin)  while target = 1, x1 > x2
        return self.weight * ((ones - person_H) + person_L).mean()
